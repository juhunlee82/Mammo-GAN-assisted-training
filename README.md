...
The figure below illustrates our key idea, increasing difficult samples by using LS and LR to convert the portion of easy lesions and easy normal tissue (a and c in Figure 1.A) as well as the difficult cases (b<sub>N</sub> and b<sub>L</sub> in Figure 1.A) to be more challenging. By doing so, we modified the score distributions from A) to C), such that the algorithm could learn key features for classification better by accessing more difficult or challenging cases. 
